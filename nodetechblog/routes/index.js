...
router.get("/fetch-comments/:blog_id",(req,res)=>{
    let {blog_id} = req.params;
    console.log(blog_id)
    let selectSQL = "select DATE_FORMAT(comment_dt,'%d,%m %Y %h:%i') as cDT,comment from comments where blog="+blog_id;
    conn.query(selectSQL,(err,rows)=>{
        if(err)
            res.send(err.message);
        res.send(rows);
...
router.post("/post-comment",(req,res)=>{
    ...
    var mont = dt.getMonth()+1;
    if(mont <10){
        mont="0"+mont;
    }
    var dat = dt.getDate();
    var tm= dt.getHours() + ":"+ dt.getMinutes();
    var commDate = `${year}-${mont}-${dat} ${tm}`
    var user = session.userSession;
    let insertSQL = `insert into comments values(null,'${comment}','${user}',${blog_id},'${commDate}')`
    conn.query(insertSQL,(err)=>{
        if(err)
            res.send(err.message);
        res.send("inserted");
